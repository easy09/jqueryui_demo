...
function getListString($catalogue,$tab=NULL){
    foreach($catalogue as $row){
        $action="<span articleid=$row[id] articletitle=$row[title] class='ui-icon ui-icon-trash' title='删除'></span><span articleid=$row[id] articletitle=$row[title] title='修改' class='ui-icon ui-icon-wrench'></span><span articleid=$row[id] articletitle=$row[title] title='分配权限' class='ui-icon ui-icon-key'></span>";
        $action.="<span articleid=$row[id] articletitle=$row[title] class='ui-icon ui-icon-trash' title='添加子分类'></span>";
        $string.=<<< STRING
        <tr>
            <td>$tab$row[title]</td>
            <td>$row[name]</td>
...
function bindModify(){
    ...
    $('.ui-icon-wrench').click(function(){
        $('#dialog').load('articlecatalogueedit.php?id='+$(this).attr('articleid'));
        var dialogOpts={
            title:'修改文章分类',
            modal:true,
            buttons:{
...
function bindResource(){
    ...
    $('.ui-icon-key').click(function(){
        $('#dialog').load('articlesresourcelevel.php?id='+$(this).attr('articleid'));
        var dialogOpts={
            title:'分配文章分类权限',
            modal:true,
            buttons:{
...
<fieldset>
    ...
    <form id="form-articlescatalogue" name="form-articlescatalogue" method="POST" action="articlescatalogue.php">
